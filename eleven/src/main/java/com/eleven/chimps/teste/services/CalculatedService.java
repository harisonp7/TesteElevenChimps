...
public class CalculatedService {
	...
	public Calculated calSkuPro(CalcDTO objDTO) {
		...
		List<SkuPromocao> listSku = repoSku.findBySku(objDTO.getSku());
		
		for (int i = 0; i < listSku.size(); i++) {
			listPercent.add(listSku.get(i).getPromocoes().get(i).getPercentual().intValue());
		 }
		for(Integer valor : listPercent) {  
		      if(valor > maxPercent) {
		            maxPercent = valor;
		      }
		}		 
		 Calculated calculated = new Calculated(objDTO.getSku(), valorOriginal, (valorOriginal + (valorOriginal*maxPercent)/100));
		 return calculated;
	}
